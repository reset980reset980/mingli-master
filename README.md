...
**웹앱 방식**의 [app.py](app.py)는 외부 AI 모델을 호출하지 않습니다. 생년월일시를 `iztro-py`로 계산한 뒤, 별/궁/사화 규칙에 따라 한국어 리포트를 자동 생성합니다. 그래서 빠르고 배포가 단순하지만, 사람처럼 깊은 맥락 추론이나 자유 질의응답은 아직 없습니다.
...
- 12궁, 주성, 보조성, 사화, 대운 계산
- 한국어 명반 HTML 생성
- 관록궁, 재백궁, 부부궁, 현재 대운의 규칙 기반 해석
- 보정 질문과 답변 입력란 제공

아직 없는 기능:

- OpenAI, OpenClaw, Claude 등 LLM API 호출
- 보정 답변을 읽고 자동으로 2차 해석을 다시 쓰는 기능
- 손금 이미지 인식
- 채팅형 추가 질문 답변

보정 질문에 답한다고 결과가 보장되지는 않습니다. 답변은 해석을 조정하는 추가 근거일 뿐입니다.
